...
export const api = {
  ...
  async loadAgentSessionHistory(sessionId: string): Promise<any[]> {
    ...
    */
   async continueClaudeCode(projectPath: string, prompt: string, model: string, tabId: string): Promise<void> {
     return apiCall("continue_claude_code", { projectPath, prompt, model, tabId });
   },
